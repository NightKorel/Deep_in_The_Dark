...
function confirmDepart() {
  ...
  window._departSelectedFood = null;
  window._departSelectedPotion = null;
  startNewDive(foodChoice, potionChoice);
}
...
function potionEffectDesc(potion, rare) {
  let e = potion.effect;
  if (e.type === "throw-damage") { let r = rare ? e.rareDmgRange : e.dmgRange; return `全體敵人受 ${r[0]}~${r[1]} 傷害`; }
...
function showPotionCraftScreen() {
  ...
  let rows = Object.keys(HERBS).map((hid) => {
    let herb = HERBS[hid];
