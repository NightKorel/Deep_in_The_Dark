...
const CASINO_GAMES = {
  ...
  mastermind: {
    name: "破譯", icon: "🔮", venue: "shelter", noBet: true,
    blurb: "H 藏了一組 4 個符號的密碼，你來猜。每猜一次他會提示「幾個對位、幾個對色不對位」。純鬥智、不靠運氣——所以不押注，破解成功直接領固定獎金，用越少次領越多。",
    start: () => mmStart(),
  },
  memory: {
    name: "記憶迴光", icon: "🌟", venue: "shelter", noBet: true,
    blurb: "H 亮出越來越長的發光符文，你照順序點回去。純技術不押注：每過一輪固定累積一點獎金，隨時可以收手領走；點錯就這局全部歸零。",
    start: () => memoryStart(),
  },
};

// 技術類遊戲固定獎金（不押注、不用賠率——因為太容易，用賠率會被無限刷）
const MM_FIXED_REWARD = { good: 12, mid: 8, low: 4 }; // 破譯：≤4次/≤6次/≤8次破解
const MEMORY_REWARD_PER_ROUND = 2; // 記憶迴光：每過一輪固定累積的獎金（收手才拿得到，點錯歸零）
...
function casinoOpenGame(gameId) {
  ...
  if (g.noBet) {
    // 技術類：不押注，直接開玩，贏了領固定獎金
    casinoRound = { gameId, bet: 0 };
...
function casinoResolveGame(payout, resultLines, outcome) {
  ...
  let noBet = gameId && CASINO_GAMES[gameId].noBet;
  let canReplay = gameId && (noBet || casinoBalance() >= 1);
  ...
  showScreen(`
    ...
    ${canReplay ? `<button class="action-btn" onclick="casinoOpenGame('${gameId}')">🔁 再玩一局</button>` : `<p class="dim">身上的${casinoCurrencyName()}見底了，改天再來吧。</p>`}
...
function mmSubmit() {
  ...
  if (fb.black === MM_LEN) {
    ...
    let reward = used <= 4 ? MM_FIXED_REWARD.good : used <= 6 ? MM_FIXED_REWARD.mid : MM_FIXED_REWARD.low;
    mmRender();
    casinoResolveGame(reward, [
...
function memoryStart() {
  memoryState = { seq: [], input: [], round: 0, pot: 0, phase: "show", done: false };
  memoryNextRound();
}
...
function memoryNextRound() {
  let s = memoryState;
  s.round++;
  // 每一輪都「重新隨機一整串」（長度=輪數），不是在舊序列後面加一個——這樣每輪前幾個不會都一樣
  s.seq = [];
  for (let i = 0; i < s.round; i++) s.seq.push(randInt(0, MEMORY_RUNES.length - 1));
  s.input = [];
  s.phase = "show";
  memoryRender();
  memoryPlaySequence();
...
// 逐一閃示序列（直接操作既有 DOM，不重繪，避免閃到一半畫面被洗掉）
...
function memoryTileClick(tile) {
  ...
  if (s.input[pos] !== s.seq[pos]) {
    ...
    setTimeout(() => casinoResolveGame(0, [
      ...
      `這局累積的 ${s.pot} 🪙 全部歸零，撐到第 ${s.round} 輪。`,
    ], "lose"), 500);
    return;
  }
  if (s.input.length === s.seq.length) {
    s.pot += MEMORY_REWARD_PER_ROUND; // 這一輪過關：固定累積獎金
    s.phase = "cleared";
    memoryRender();
  } else {
...
function memoryCashout() {
  ...
  s.done = true;
  casinoResolveGame(s.pot, [
    `你穩穩記到第 ${s.round} 輪就收手，帶走累積的 ${s.pot} 🪙。`,
  ], s.pot > 0 ? "win" : "push");
}

function memoryRender() {
  let s = memoryState;
  let phaseText = s.phase === "show" ? "👀 看好囉，記住順序……"
    : s.phase === "input" ? `👇 換你！照順序點回去　已點 ${s.input.length} / ${s.seq.length}`
    : `✅ 第 ${s.round} 輪過關！`;
  ...
  showScreen(`
    ...
    <div class="card mines-status">
      <span>第 ${s.round} 輪</span>
      <span>累積獎金 <b>${s.pot} 🪙</b></span>
      <span>下一輪 +${MEMORY_REWARD_PER_ROUND}</span>
    </div>
    <div class="card" style="text-align:center;">
    ...
    <div class="hl-guess-row">
      <button class="action-btn" onclick="memoryNextRound()">➡️ 再來一輪（+${MEMORY_REWARD_PER_ROUND}）</button>
      <button class="action-btn secondary" onclick="memoryCashout()">💰 收手（帶走 ${s.pot} 🪙）</button>
    </div>` : ""}
  `, { withTopbar: !!activeDive });
}
