...
const CASINO_GAMES = {
  ...
  mastermind: {
    name: "破譯", icon: "🔮", venue: "shelter",
    blurb: "H 藏了一組 4 個符號的密碼，你來猜。每猜一次他會提示「幾個對位、幾個對色不對位」。用越少次破解，賠得越多。純鬥智、不靠運氣。",
    start: (bet) => mmStart(bet),
  },
  memory: {
    name: "記憶迴光", icon: "🌟", venue: "shelter",
    blurb: "H 會亮出一串發光的符文，你照順序點回去。過一輪、序列變長、倍率翻倍，每輪之間可以收手；點錯就全輸。考驗記憶力。",
    start: (bet) => memoryStart(bet),
  },
};
...
// 用越少次破解賠越多（token-only，走「穩定但天花板低」路線；8 次才破＝保本）
const MM_PAYOUT_MULT = { 1: 6, 2: 5, 3: 4, 4: 2.5, 5: 1.8, 6: 1.4, 7: 1.15, 8: 1.0 };

function mmStart(bet) {
  let code = [];
  for (let i = 0; i < MM_LEN; i++) code.push(randInt(0, MM_SYMBOLS.length - 1));
  mmState = { bet, code, guesses: [], current: [0, 0, 0, 0], done: false };
  mmRender();
}
...
function mmSubmit() {
  ...
  if (fb.black === MM_LEN) {
    ...
    let mult = MM_PAYOUT_MULT[used] || 1;
    mmRender();
    casinoResolveGame(Math.floor(s.bet * mult), [
...
function memoryStart(bet) {
  memoryState = { bet, seq: [], input: [], round: 0, mult: 1, phase: "show", done: false };
  memoryNextRound();
}

function memoryMultForRound(round) {
  return round <= 0 ? 1 : Math.pow(1.4, round - 1); // 過第1輪保本，之後每輪×1.4
}

function memoryNextRound() {
  let s = memoryState;
  s.round++;
  s.seq.push(randInt(0, MEMORY_RUNES.length - 1));
  s.input = [];
  s.phase = "show";
  memoryRender();
  memoryPlaySequence();
...
function memoryTileClick(tile) {
  ...
  if (s.input.length === s.seq.length) {
    // 這一輪過關
    s.mult = memoryMultForRound(s.round);
    s.phase = "cleared";
    memoryRender();
  }
}
...
function memoryCashout() {
  ...
  casinoResolveGame(payout, [
    `你穩穩記到第 ${s.round} 輪就收手了。`,
    `倍率 ×${s.mult.toFixed(2)}。`,
  ], payout > s.bet ? "win" : "push");
}

function memoryRender() {
  let s = memoryState;
  let payout = Math.floor(s.bet * memoryMultForRound(s.round));
  let phaseText = s.phase === "show" ? "👀 看好囉，記住順序……"
    : s.phase === "input" ? `👇 換你！照順序點回去（${s.input.length}/${s.seq.length}）`
    : `✅ 第 ${s.round} 輪過關！`;
  ...
  let nextMult = memoryMultForRound(s.round + 1);
  showScreen(`
    <h2 class="screen-title">🌟 記憶迴光</h2>
    <div class="card mines-status">
      <span>第 ${s.round} 輪</span>
      <span>目前倍率 <b>×${s.mult.toFixed(2)}</b></span>
      ${s.phase === "cleared" ? `<span>可收 ${Math.floor(s.bet * s.mult)} ${casinoCurrencyIcon()}</span>` : ""}
    </div>
    <div class="card" style="text-align:center;">
    ...
    <div class="hl-guess-row">
      <button class="action-btn" onclick="memoryNextRound()">➡️ 再來一輪（下一輪 ×${nextMult.toFixed(2)}）</button>
      <button class="action-btn secondary" onclick="memoryCashout()">💰 收手（帶走 ${Math.floor(s.bet * s.mult)} ${casinoCurrencyIcon()}）</button>
    </div>` : ""}
  `, { withTopbar: !!activeDive });
}
