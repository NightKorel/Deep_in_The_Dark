...
function executeEnemySkill(enemy) {
  ...
  if (skill.type === "attack") {
    ...
    logBattle(`${enemy.icon} ${enemy.name} 使用「${skill.name}」，回復了 ${heal} 點血量。`);
...
function handleBattleWin() {
  ...
  let crystalEarned = 0, expEarned = 0, foodDrops = [], foodDropsText = "";
  ...
  renderBattleScreen();
  if (mimicDefeated) {
    setTimeout(() => resolveMimicBonus(crystalEarned, expEarned, foodDrops, foodDropsText, fallenIds), 900);
    return;
  }
  setTimeout(() => endBattle("win", { crystalEarned, expEarned, foodDrops, foodDropsText, fallenIds }), 900);
}

// 混戰中打死寶箱怪的額外獎勵，疊加在原本的戰鬥獎勵之上：50%潛晶大獎、50%潛晶小獎+1個隨機遺物
function resolveMimicBonus(crystalEarned, expEarned, foodDrops, foodDropsText, fallenIds) {
  if (chance(0.5)) {
    let bonus = randInt(MIMIC_BONUS_CRYSTAL_BIG[0], MIMIC_BONUS_CRYSTAL_BIG[1]);
    crystalEarned += bonus;
    systemToast(`💎 寶箱怪噴出一大筆潛晶！+${bonus}`);
    endBattle("win", { crystalEarned, expEarned, foodDrops, foodDropsText, fallenIds });
  } else {
